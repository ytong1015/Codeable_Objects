...
public class Geom {
	...
	public static Point polarToCart(double r, double theta) {
		...
		/*
		 * double[] xY = new double[2]; xY[0]=x; xY[1] = y; return xY;
		 */
	...
	public static double getHighestAngle(DCHalfEdge edge, Point focus) {
		...
		return Geom.cartToPolar(target.getX() - focus.getX(), target.getY()
				- focus.getY())[1];
	}

	public static DCHalfEdge getIntersectedEdge(Point focus, Point direction,
			DCHalfEdge parentEdge, DoublyConnectedEdgeList border) {
		...
		double[] thetas = border.getBorderPoints(focus); // degrees of all
															// points along the
															// border
		...
		for (int i = 0; i < thetas.length; i++) { // iterate through all edges
			...
			if (thetas[after] > thetas[i]) {// special case where angle of
											// previous edge is greater than
											// angle of current edge
				// myParent.println("call special case");
				if (edgeTheta >= thetas[after] || edgeTheta < thetas[i]) {// detects
																			// quadrant
																			// of
																			// intersection
																			// for
					 ...
					 * myParent
					 * .println("start x,y,="+focus.getX()+","+focus.getY());
					 * myParent
					 * .println("end x,y,="+direction.getX()+","+direction
					 * .getY());
					 * myParent.println("intersection x,y,="+intersection
					 * .getX()+","+intersection.getY()+"\n\n\n");
					 */
			...
			} else {// otherwise all thetas should be greater than the
					// preceeding theta

				if (edgeTheta <= thetas[i] && edgeTheta > thetas[after]) {// detects
																			// quadrant
																			// of
																			// intersection
	...
	public static Point getIntersectedEdgePoint(Point focus, Point direction,
			DCHalfEdge parentEdge, DCHalfEdge borderEdge) {
	...
	public static DoublyConnectedEdgeList linesToDCEdgeList(LineCollection lc) {
		...
		for (int i = 0; i < l.size(); i++) {
			p.addHalfEdge(new DCHalfEdge(l.get(i).start.copy(), l.get(i).end
					.copy()));
		}
		return p;
	...
	public static char rayTypePointInPolygon(Point q, DoublyConnectedEdgeList p) {
		...
		boolean Rstrad, Lstrad; // flags that indicate the edge straddles the x
								// axis
		// Vector<CompPoint> p = new Vector<CompPoint>();

		// for each edge in poly, see if crosses rays
		/*
		 * for(i=0;i<n;i++){ DCHalfEdge edge = p.edge.get(i); CompPoint newPoint
		 * = new
		 * CompPoint(edge.start.getX()-q.getX(),edge.start.getY()-q.getY());
		 * 
		 * }
		 */
		...
		/*
		 * i1=(i+n-1)%n; DCHalfEdge edge1 = p.edges.get(i1); Rstrad =
		 * (edge.start.getY()>q.getY())!=(edge1.start.getY()>q.getY()); Lstrad =
		 * (edge.start.getY()<q.getY())!=(edge1.start.getY()<q.getY());
		 * 
		 * if(Rstrad|| Lstrad){ double x =
		 * (edge.start.getX()*edge1.start.getY()-
		 * edge1.start.getX()*edge.start.getY
		 * ())/(edge1.start.getY()-edge.start.getY());
		 * 
		 * if(Rstrad && x> q.getX()){ Rcross++; } if(Lstrad && x> q.getX()){
		 * Lcross++; }
		 * 
		 * } }
		 */
	...
	public static boolean pointInComPolygon(Point testPoint,
			DoublyConnectedEdgeList border) {
		int angleCount = 0;
	...
	// determines if a given point is in a polygon defined by a doubly connected
	// edge list
	// only works for regular polygons
	public static boolean pointInPolygon(Point testPoint,
			DoublyConnectedEdgeList border) {
	...
	// determines if a segment intersects a polygon defined by a doubly
	// connected edge list and returns edges of intersection if they exist
	public static Vector<Line> edgeIntersectsPolygon(Line edge,
			Vector<Line> border) {
		Vector<Line> intersectedEdges = new Vector<Line>();
		for (int i = border.size() - 1; i >= 0; i--) {
			Line borderEdge = border.get(i);
			if (lineIntersect(borderEdge.start, borderEdge.end, edge.start,
					edge.end)) {
	...
	// determines if a segment intersects a polygon defined by a doubly
	// connected edge list and returns edges of intersection if they exist
	public static Vector<DCHalfEdge> edgeIntersectsPolygon(DCHalfEdge edge,
			DoublyConnectedEdgeList border) {
	...
	// detects if a line intersects an edge but does not return the point(s) of
	// intersection
	public static boolean lineIntersect(Point edgeStart, Point edgeEnd,
			Point testPointStart, Point testPointEnd) {
	...
	public static boolean between(Point edgeStart, Point edgeEnd,
			Point testPoint) {
		if (!collinear(edgeStart, edgeEnd, testPoint)) {
			return false;
		}
		if (edgeStart.getX() != edgeEnd.getX()) {
			return (edgeStart.getX() <= testPoint.getX())
					&& (testPoint.getX() <= edgeEnd.getX())
					|| (edgeStart.getX() >= testPoint.getX())
					&& (testPoint.getX() >= edgeEnd.getX());
		} else {
			return (edgeStart.getY() <= testPoint.getY())
					&& (testPoint.getY() <= edgeEnd.getY())
					|| (edgeStart.getY() >= testPoint.getY())
					&& (testPoint.getY() >= edgeEnd.getY());
		}

	}

	public static boolean intersectProp(Point edgeStart, Point edgeEnd,
			Point testPointStart, Point testPointEnd) {
		...
		return Xor(left(edgeStart, edgeEnd, testPointStart),
				left(edgeStart, edgeEnd, testPointEnd))
				&& Xor(left(testPointStart, testPointEnd, edgeStart),
						left(testPointStart, testPointEnd, edgeEnd));
	...
	public static boolean collinear(Point edgeStart, Point edgeEnd,
			Point testPoint) {
		return area2(edgeStart, edgeEnd, testPoint) == 0;
	...
	// finds the point of intersection between two edges that are known to
	// intersect
	public static Point findIntersectionPoint(DCHalfEdge edge,
		...
		if (Double.isInfinite(borderEdge.getSlope())) {// check to see if slope
														// is undefined (line is
														// vertical)

			mx = borderEdge.start.getX();
			my = (mx * edge.getSlope()) + edge.getYIntercept();
		} else if (Double.isNaN(borderEdge.getSlope())) {// check to see if
															// slope is NaN
															// (line is
															// horizontal)
			...
			if (Double.isInfinite(edge.getSlope())) {
				...
				mx = (edge.getYIntercept() - borderEdge.getYIntercept())
						/ (borderEdge.getSlope() - edge.getSlope());// line has
																	// a slope
				my = (mx * edge.getSlope()) + edge.getYIntercept();
			}
		}
	...
	public static Point findIntersectionPoint(DCHalfEdge borderEdge,
		...
		if (Double.isInfinite(borderEdge.getSlope())) {// check to see if slope
			...
			mx = (b - borderEdge.getYIntercept()) / (borderEdge.getSlope() - m);// line
																				// has
																				// a
																				// slope
			my = (mx * m) + b;
	...
	public static Point closestPoint(Vec2d seg_a, Vec2d seg_b,
		...
		Vec2d pt_v = new Vec2d(circleOrigin.getX(), circleOrigin.getY())
				.sub(seg_a);
	...
	public static double SignedPolygonArea(DoublyConnectedEdgeList poly) {
		...
		for (i = 0; i < N; i++) {
			j = (i + 1) % N;
			area += poly.edges.get(i).start.getX()
					* poly.edges.get(j).start.getY();
			area -= poly.edges.get(i).start.getY()
					* poly.edges.get(j).start.getX();
		}
		area /= 2.0;
	...
	public static Point findCentroid(DoublyConnectedEdgeList polygon) {
		...
		for (i = 0; i < verticies.size(); i++) {
			...
			factor = (verticies.get(i).getX() * verticies.get(j).getY() - verticies
					.get(j).getX() * verticies.get(i).getY());
	...
	public static Vector<Point> removeDuplicateVerts(
			DoublyConnectedEdgeList poly) {
		Vector<Point> verticies = new Vector<Point>(0);
	...
	public static Vector<Point> removeDuplicateVerts(Vector<Point> verticies) {
		 ...
		 * 
		 * for(int i=0;i<verticies.size();i++){ for(int
		 * j=0;j<verticies.size();j++){ if(j!=i){
