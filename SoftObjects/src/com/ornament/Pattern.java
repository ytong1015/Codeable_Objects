package com.ornament;

import java.util.Vector;

import com.datatype.DCEL;
import com.datatype.Point;
import com.primitive2d.Ellipse;
import com.primitive2d.Intersection;
import com.primitive2d.Line;
import com.primitive2d.LineCollection;
import com.primitive2d.Polygon;

//base class for all patterns
public class Pattern extends LineCollection {

	protected double RADIUS = 2.5F; // /////////////////// NEED?
	private DCEL dcel;

	public Pattern() {
		super();
		this.r = 255;
		this.g = 0;
		this.b = 255;

	}

	public Pattern(Point origin, Vector<Point> points, Vector<Line> lines,
			Vector<Polygon> polygons, Vector<Ellipse> ellipses,
			Vector<Intersection> intersections) {
		super(origin, points, lines, polygons, ellipses, intersections);
		this.r = 255;
		this.g = 0;
		this.b = 255;

	}

	public Intersection createIntersection(double x, double y) {
		Intersection intersection = new Intersection(x, y, RADIUS);
		return intersection;

	}

	@Override
	public void addPolygon(Polygon poly) {
		this.addAllLines(poly.copyAllLines());
	}

	// ////////////////////////////////////////////////////////
	public void addPolygon(Vector<int[]> points) {
		this.addPolygon(points);
	}

	@Override
	public Pattern copy() {
		Vector<Line> lines = new Vector<Line>();
		Vector<Point> points = new Vector<Point>();
		Vector<Polygon> polygons = new Vector<Polygon>();
		Vector<Ellipse> ellipses = new Vector<Ellipse>();
		Vector<Intersection> intersections = new Vector<Intersection>();
		Point newOrigin = this.origin.copy();

		for (int i = 0; i < this.getAllPolygons().size(); i++) {
			Vector<Line> oldPolygonLines = getPolygonAt(i).getAllLines();
			Polygon polygon = new Polygon();

			for (int j = 0; j < oldPolygonLines.size(); j++) {
				polygon.addLine(oldPolygonLines.get(j).copy());
			}
			polygons.add(polygon);
		}

		for (int i = 0; i < this.getAllLines().size(); i++) {
			Line line = this.getAllLines().get(i).copy();
			lines.add(line);
			points.add(line.start);
			points.add(line.end);
		}

		for (int i = 0; i < this.getAllPoints().size(); i++) {
			Point point = points.get(i).copy();
			points.add(point);
		}

		// will need to implement ellipse call

		Pattern newPattern = new Pattern(newOrigin, points, lines, polygons,
				ellipses, intersections);

		// newPattern.reLinkLines();

		return newPattern;

	}

	public void addCopy(LineCollection lc) {
		this.addAllLines(lc.copyAllLines());
	}

	public void clipTo(Polygon poly) {
		poly.clipPattern(this);
	}

	// ////////// add on methods
	public void setDcel(DCEL dcel) {
		this.dcel = dcel;
	}

	public DCEL getDcel() {
		return dcel;
	}

}
